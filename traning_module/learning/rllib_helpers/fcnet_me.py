...
class MyFullyConnectedNetwork(TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config,
        ...
        custom_std = custom_model_config.get("custom_action_std")
        #custom_std = model_config.get("custom_model_config").get("custom_action_std")

        if custom_std is None:
            log_std = 0.0
        else:
            log_std = np.log(custom_std)
        ...
        if free_log_std:
            ...
            """
            TODO: pass in initial standard deviation 
            """
            self.log_std_var = tf.Variable(
                # [0.0] * num_outputs, dtype=tf.float32, name="log_std")
                # [-2.0] * num_outputs, dtype=tf.float32, name="log_std")
                [log_std] * num_outputs, dtype=tf.float32, name="log_std")
