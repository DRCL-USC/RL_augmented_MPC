...
template <typename y_t, typename x_t>
y_t cubicBezier_forStair(y_t y0, y_t yf, x_t x) {
  static_assert(std::is_floating_point<x_t>::value,
                "must use floating point value");
  assert(x >= 0 && x <= 1);
  y_t yDiff = yf - y0;
  x_t bezier = x * x * x + x_t(0.1) * (x * x * (x_t(1) - x));
  return y0 + bezier * yDiff;
}

template <typename y_t, typename x_t>
y_t cubicBezierFirstDerivative_forStair(y_t y0, y_t yf, x_t x) {
  static_assert(std::is_floating_point<x_t>::value,
                "must use floating point value");
  assert(x >= 0 && x <= 1);
  y_t yDiff = yf - y0;
  x_t bezier = x_t(0.2) * x * (x_t(1) - x);
  return bezier * yDiff;
}
