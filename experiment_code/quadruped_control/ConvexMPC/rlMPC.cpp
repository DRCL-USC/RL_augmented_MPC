...
/* =========================== Controller ============================= */
rlMPC::rlMPC(double _dt, int _iterations_between_mpc, int horizon) : 
  MPCLocomotion(_dt, _iterations_between_mpc, horizon),
  solver(_dt * _iterations_between_mpc, horizon, 0.4, 450),
  policy(82, 18, "../policies/foot_reaction_test.json")
{
  contact.setZero();
  for(int i = 0; i < 8; i++)
  {
...
void rlMPC::run(ControlFSMData &data)
{
  ...
  for (int foot = 0; foot < 4; foot++)
  {
    ...
    if (swingState > 0) // foot is in swing
    {
      ...
      data._legController->commands[foot].kpJoint.diagonal() << 30, 30, 30;
...
void rlMPC::updateMPCIfNeeded(int *mpcTable, ControlFSMData &data)
{
  if ((iterationCounter % 30) == 0)
  {
	...
	policy.output_action.setZero();
    policy.current_obs.block(0, 0, 1, 3) = seResult.rpy.transpose();
    policy.current_obs.block(0, 3, 1, 3) = seResult.vWorld.transpose();
    policy.current_obs.block(0, 6, 1, 3) = seResult.omegaWorld.transpose();
    for(int i = 0; i < 4; i++)
    {
      policy.current_obs.block(0, 9 + 3*i, 1, 3) = data._legController->data[i].q.transpose();
      policy.current_obs.block(0, 21 + 3*i, 1, 3) = data._legController->data[i].qd.transpose();
    }
    policy.current_obs.block(0, 33, 1, 8) <<  foothold_heuristic[0], foothold_heuristic[1],
    ...
    policy.current_obs.block(0, 41, 1, 4) = contact;
    policy.current_obs.block(0, 45, 1, 4) <<  mpcTable[0], mpcTable[1], mpcTable[2], mpcTable[3]; //change training later
    for(int i = 0; i < 4; i++)
    {
      policy.current_obs.block(0, 49+i*3, 1, 3) = f_ff[i]/12.0;
    }
    policy.current_obs.block(0, 61, 1, 3) << v_des_world[0], v_des_world[1], stateCommand->data.stateDes[11];
    policy.current_obs.block(0, 64, 1, 18) << policy.output_action;
    
   // std::cout << "observation " << policy.current_obs << std::endl;
   // std::cout << " compute action " << std::endl;
    policy.update_observation();
    policy.update_action();

  //  std::cout << " output " << std::endl;
  //  std::cout << policy.output_action << std::endl;
    solver.set_acclerations(policy.output_action.block(0, 0, 1, 3).transpose(), policy.output_action.block(0, 3, 1, 3).transpose());
    for(int i = 0; i < 4; i++){
      data._legController->q_offset[i] = policy.output_action.block(0, 6 + 3*i, 1, 3).transpose();
//      std::cout << " q offset " << i << "\n"  << data._legController->q_offset[i] << "\n"; 
    }
    Timer t1;
    t1.start();
